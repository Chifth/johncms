...
class DownloadsUtils
{
     ...
     */
    public function setMetaForTopic(DownloadsTopic $topic): void
    {
    ...
    {
        $section = DownloadsSection::query()->find($sectionId);
        $sections[] = $section;
        if ($section->parent) {
        ...
        $message = preg_replace_callback(
            ...
            static function ($link) {
                ...
                if ($parsed_url['host'] === $host && ! empty($query_params['id']) && ! empty($query_params['type']) && $query_params['type'] === 'topic') {
                    try {
                        $topic = (new DownloadsTopic())->findOrFail($query_params['id']);
                        $name = strtr(
                            $topic->name,
                            [
                                '&quot;' => '',
    ...
    public static function getPostPage(int $post_id, int $topic_id): string
    {
        /** @var User $user */
        $user = di(User::class);
        $upfp = $user->set_downloads['upfp'] ?? 0;
        $message = (new DownloadsMessage())
            ->where('topic_id', '=', $topic_id)
            ->where('downloads_messages.id', empty($upfp) ? '<=' : '>=', $post_id)
            ->orderBy('id', empty($upfp) ? 'ASC' : 'DESC')
            ->paginate($user->config->kmess);

        $page = $message->lastPage();

        return '/downloads/?type=topic&id=' . $topic_id . ($page > 1 ? '&page=' . $page : '');
    }

    // Вспомогательная Функция обработки ссылок форума
    public static function downloadsLink($m)
    {
        global $db, $config;
        ...
        if ('http://' . $p['host'] . ($p['path'] ?? '') . '?id=' == $config['homeurl'] . '/downloads/?id=') {
            $thid = abs((int) (preg_replace('/(.*?)id=/si', '', $m[3])));
            $req = $db->query("SELECT `name` FROM `downloads_topic` WHERE `id`= '${thid}' AND (`deleted` != '1' OR deleted IS NULL)");
