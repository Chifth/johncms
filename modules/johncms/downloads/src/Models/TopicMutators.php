...
trait TopicMutators
{
    ...
    {
        return route('downloads.topic', ['id' => $this->id, 'topicName' => Str::slug($this->name)]);
    }
    ...
    public function getLastPageUrlAttribute(): string
    {
        ...
        return route('downloads.topic', ['id' => $this->id, 'topicName' => Str::slug($this->name)], $query);
    }
    ...
    public function getCalculatedMetaDescriptionAttribute(): string
    {
        ...
        $config = di('config')['downloads']['settings'];
