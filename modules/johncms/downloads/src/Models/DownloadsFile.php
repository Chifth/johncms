...
class DownloadsFile extends Model
{
    ...
    protected static function boot(): void
    {
        ...
        static::addGlobalScope(
            ...
            static function (Builder $builder) {
                /** @var User $user */
                $user = di(User::class);
                if ($user?->hasPermission(DownloadsPermissions::MANAGE_FILES)) {
                    $builder->where('del', '!=', 1);
                }
            }
        );
    ...
    {
        $this->file_info = new FileInfo(UPLOAD_PATH . 'downloads/files/' . $this->filename);
    }
    ...
    public function getFilePreviewAttribute(): string
    {
        ...
        if ($this->file_info->isImage()) {
            return '/assets/modules/downloads/thumbinal.php?file=' . (urlencode($this->filename));
        }
        return '';
    }
    ...
    {
        return '/downloads/?act=file&amp;id=' . $this->id;
    }
    ...
    {
        return '/downloads/?act=editpost&amp;do=delfile&amp;fid=' . $this->id . '&amp;id=' . $this->post;
    }
}
